...
FUNCTION computeBranchWeight RETURNS INTEGER (pcPgm AS CHARACTER, piWeight AS INTEGER) FORWARD.
FUNCTION findUnbalancedBranch RETURNS CHARACTER (pcPgm AS CHARACTER) FORWARD.
...
DEFINE VARIABLE cUnbalancedBranch AS CHARACTER   NO-UNDO.

DEFINE TEMP-TABLE ttPgm NO-UNDO
    FIELD cPgm AS CHARACTER
    FIELD iWeight AS INTEGER
    FIELD cFather AS CHARACTER
    FIELD iBranchWeight AS INTEGER
    FIELD cUnbalancedBranch AS CHARACTER FORMAT "X(20)"
    INDEX ix IS PRIMARY cPgm
    INDEX ixf cFather
    INDEX ixb cUnbalancedBranch.
...
DO iLine = 1 TO NUM-ENTRIES(cInput, "~n"):
    ...
    END.
END.
/* compute branches weight */
/* start from the root */
FIND ttPgm WHERE ttPgm.cFather = "".
cPgm = ttPgm.cPgm.
ttPgm.iBranchWeight = computeBranchWeight(ttPgm.cPgm, ttPgm.iWeight).
MESSAGE ttPgm.cPgm ttPgm.iBranchWeight
    VIEW-AS ALERT-BOX INFO BUTTONS OK.
/* find the un-balanced branches */
ttPgm.cUnbalancedBranch = findUnbalancedBranch(cPgm).
/*FOR EACH ttPgm WHERE ttPgm.cUnbalancedBranch <> ?:
    DISP ttPgm.
END.*/
/* starting from the root, find the last unbalanced branch */
DO WHILE TRUE:
    FIND ttPgm WHERE ttPgm.cFather = cPgm AND ttPgm.cUnbalancedBranch <> ? NO-ERROR.
    IF NOT AVAILABLE ttPgm THEN LEAVE.
    cPgm = ttPgm.cPgm.
    cUnbalancedBranch = ttPgm.cUnbalancedBranch.
END.
FIND ttPgm WHERE ttPgm.cPgm = ENTRY(1, cUnbalancedBranch).
MESSAGE cUnbalancedBranch SKIP
    "actual weight:" ttPgm.iWeight SKIP
    "fixed weight:" ttPgm.iWeight - INTEGER(ENTRY(2,cUnbalancedBranch)) + INTEGER(ENTRY(3,cUnbalancedBranch))
    VIEW-AS ALERT-BOX INFO BUTTONS OK.

FUNCTION computeBranchWeight RETURNS INTEGER (pcPgm AS CHARACTER, piWeight AS INTEGER):
    DEFINE VARIABLE iWeight AS INTEGER     NO-UNDO.
    DEFINE BUFFER ttPgm FOR ttPgm.
    iWeight = piWeight.
    FOR EACH ttPgm WHERE ttPgm.cFather = pcPgm:
        ttPgm.iBranchWeight = computeBranchWeight(ttPgm.cPgm, ttPgm.iWeight).
        iWeight = iWeight + ttPgm.iBranchWeight.
    END.
    RETURN iWeight.
END FUNCTION.

FUNCTION findUnbalancedBranch RETURNS CHARACTER (pcPgm AS CHARACTER):
    DEFINE BUFFER ttPgm FOR ttPgm.
    DEFINE VARIABLE iW AS INTEGER  EXTENT 10   NO-UNDO.
    DEFINE VARIABLE cP AS CHARACTER EXTENT 10  NO-UNDO.
    DEFINE VARIABLE iBranch AS INTEGER  NO-UNDO.
    DEFINE VARIABLE iBranches AS INTEGER  NO-UNDO.
    DEFINE VARIABLE iWeight1 AS INTEGER     NO-UNDO.
    DEFINE VARIABLE iWeight2 AS INTEGER     NO-UNDO.
    DEFINE VARIABLE inb1 AS INTEGER     NO-UNDO.
    DEFINE VARIABLE inb2 AS INTEGER     NO-UNDO.
    DEFINE VARIABLE cPgm1 AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE cPgm2 AS CHARACTER   NO-UNDO.
    FOR EACH ttPgm WHERE ttPgm.cFather = pcPgm:
        iBranches = iBranches + 1.
        ASSIGN
            iW[iBranches] = ttPgm.iBranchWeight
            cP[iBranches] = ttPgm.cPgm.
        ttPgm.cUnbalancedBranch = findUnbalancedBranch(ttPgm.cPgm).
    END.
    ASSIGN
        iWeight1 = iW[1]
        cPgm1 = cP[1]
        inb1 = 1.
    DO iBranch = 2 TO iBranches:
        IF iW[iBranch] = iWeight1 THEN ASSIGN
            inb1 = inb1 + 1.
        ELSE ASSIGN
            cPgm2 = cP[iBranch]
            iWeight2 = iW[iBranch]
            inb2 = inb2 + 1.
    END.
    IF inb1 + inb2 > 1 AND inb2 > 0 THEN DO:
        IF inb1 = 1 THEN
            RETURN cPgm1 + "," + STRING(iWeight1) + "," + STRING(iWeight2).
        ELSE IF inb2 = 1 THEN
            RETURN cPgm2 + "," + STRING(iWeight2) + "," + STRING(iWeight1).
        ELSE /* should never happen */
            DISP iW SKIP pcPgm cPgm1 inb1 cPgm2 inb2.
    END.
END FUNCTION.
